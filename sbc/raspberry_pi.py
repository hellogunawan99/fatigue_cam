...
YAWN_THRESH = 15  # sesuai jarak wajah wajar, karna untuk ngukur jarak bibir atas sama bawah
alarm_status = False
alarm_status2 = False
saying = False
COUNTER = 0
...
# detector = dlib.get_frontal_face_detector()       # more slower for sbc or Raspberry Pi
...
vs = VideoStream(src=args["webcam"]).start()      # for Computer
...
while True:
    ...
    # rects = detector(gray, 0)     $ for Computer
    rects = detector.detectMultiScale(gray, scaleFactor=1.1,
                                      minNeighbors=5, minSize=(30, 30),
                                      flags=cv2.CASCADE_SCALE_IMAGE)

    # for rect in rects:    # for Computer
    for (x, y, w, h) in rects:      # for sbc or Raspberry Pi
